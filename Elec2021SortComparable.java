...
/**
* The Elec2021SortComparable runs an application that will be used to give examples
...
* @since   2021-05-15 
*/
public class Elec2021SortComparable{
	int n = 0;
	Elector elector; 
	GetFromConsole gfc;
    ...
    */
	private void printVoters() throws ClassNotFoundException,IllegalArgumentException{
		...
		if(elecList.get(index).getClass().getName() == "societyAperson.Elector"){
			...
			//sorted elecList (new)
			eleObjList.stream().sorted().forEach(x -> {
				try{
					pr.printObject(x);
   ...
   */
	public static void main(String[] arg)throws IOException,InterruptedException,NoSuchFieldException,IllegalArgumentException, IllegalAccessException,ClassNotFoundException{
		Elec2021SortComparable elec2021 = new Elec2021SortComparable();
...
java societyAperson.Elec2021SortComparable
